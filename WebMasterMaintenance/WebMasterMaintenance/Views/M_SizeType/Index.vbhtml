@ModelType IEnumerable(Of WebMasterMaintenance.M_SizeType)
@Code
    ViewData("Title") = "Index"
End Code

<h2>M_SizeTypeView</h2>

<p>
    @Html.ActionLink("Create New", "Create")
</p>
<table class="table">
    <tr>
        <th>
            @Html.DisplayNameFor(Function(model) model.SizeTypeName)
        </th>
        <th>
            @Html.DisplayNameFor(Function(model) model.InsertedBy)
        </th>
        <th>
            @Html.DisplayNameFor(Function(model) model.InsertedAt)
        </th>
        <th>
            @Html.DisplayNameFor(Function(model) model.UpdatedBy)
        </th>
        <th>
            @Html.DisplayNameFor(Function(model) model.UpdateAt)
        </th>
        <th></th>
    </tr>

@For Each item In Model
    @<tr>
        <td>
            @Html.DisplayFor(Function(modelItem) item.SizeTypeName)
        </td>
        <td>
            @Html.DisplayFor(Function(modelItem) item.InsertedBy)
        </td>
        <td>
            @Html.DisplayFor(Function(modelItem) item.InsertedAt)
        </td>
        <td>
            @Html.DisplayFor(Function(modelItem) item.UpdatedBy)
        </td>
        <td>
            @Html.DisplayFor(Function(modelItem) item.UpdateAt)
        </td>
        <td>
            @Html.ActionLink("Edit", "Edit", New With {.id = item.SizeTypeCd }) |
            @Html.ActionLink("Details", "Details", New With {.id = item.SizeTypeCd }) |
            @Html.ActionLink("Delete", "Delete", New With {.id = item.SizeTypeCd })
        </td>
    </tr>
Next

</table>
